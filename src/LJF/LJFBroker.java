...
public class LJFBroker extends DatacenterBroker {
	...
	public LJFBroker(String name,Calculator calculator) throws Exception {
		super(name);
	...
	public void startEntity() {
		...
		int pesNumber = 1;
		String vmm = "Xen";
		for (int i = 0; i < vmEachTypeCount * 4; i++) {
			int newVmId = i;
	...
	private void schdule() {
		if(readyCloudlet.size()!=0){
			readyCloudlet.sort(new Comparator<Cloudlet>() {
                @Override
            ...
            Collections.reverse(readyCloudlet);
			send(getId(), 120.0, Constants.SCHDULE);
		}
	}
